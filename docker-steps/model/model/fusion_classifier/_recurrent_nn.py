...
class FCRecurrentNN(MyModel):
    def __init__(
            self,
            model_dir: str,
            model_name: str,
            config: FCRecurrentNNConfig = None,
            n_classes: int = 1,
        ...
        self.fusion_classifier = nn.LSTM(
            input_size=self.gene_classifier.config.hyper_parameters['hidden_size'],
        ...
        self.projection = nn.Sequential(
            nn.Linear(
                in_features=config.n_sentences * self.gene_classifier.config.hyper_parameters['hidden_size'],
    ...
    def forward(
        ...
        # execute fusion classifier
        outputs, _ = self.fusion_classifier(inputs)

        # execute classification layer
